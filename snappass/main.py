...
SMTP_SERVER = "smail.alarislabs.com"
SMTP_FROM = "otp@alarislabs.com"
...
def log_request_info():
    ...
    logging.info(f"Source IP: {request.headers.get('X-Real-IP', 'Not provided')}")
    #Uncomment to see password in logs
    #if request.is_json:
    #    logging.info(f"Request JSON body: {request.json}")
    #Uncomment to see password in logs
    #elif request.form:
    #    logging.info(f"Request form data: {request.form}")
    #else:
    #    logging.warning("Request does not contain JSON or form data.")
...
def log_response_info(response):
    ...
    if response.status_code == 200:
        ...
        if content_type == "application/json":
            try:
                logging.info(f"Response JSON body: {response.get_json()}")
            except Exception as e:
                logging.warning(f"Failed to parse JSON: {e}")
        else:
            logging.info("Response content: Skipping non-JSON content.")
